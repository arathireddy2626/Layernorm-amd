...
torch::Tensor compute_denominator_welford_hip(
    torch::Tensor x,
    c10::optional<int64_t> stream_ptr
);

// V3: Welford + fused normalize + 512 threads
void denominator_normalize_welford_512_hip(
    torch::Tensor x,
    torch::Tensor raw_output,
    torch::Tensor b_new,
    int h,
...
void rmsnorm_normalize_512_hip(
    torch::Tensor x,
    torch::Tensor raw_output,
    torch::Tensor b_new,
    int h,
...
torch::Tensor compute_denominator(
    ...
    return compute_denominator_hip(x, stream_ptr);
}
...
void denominator_normalize(
    ...
    denominator_normalize_hip(x, raw_output, b_new, h, eps);
}
...
torch::Tensor compute_denominator_welford(
    ...
    return compute_denominator_welford_hip(x, stream_ptr);
}
...
void denominator_normalize_welford_512(
    ...
    denominator_normalize_welford_512_hip(x, raw_output, b_new, h, eps);
}
...
void rmsnorm_normalize(
    ...
    rmsnorm_normalize_hip(x, raw_output, b_new, h, eps);
}
...
void rmsnorm_normalize_512(
    ...
    rmsnorm_normalize_512_hip(x, raw_output, b_new, h, eps);
}
