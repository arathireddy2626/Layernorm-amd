...
from src.load_hip import denominator_hip
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with _nvtx_range("FusedLNLinear_V0"):
            ...
            with torch.cuda.stream(self.denom_stream):
                v = denominator_hip.compute_denominator(x_2d)
...
class FusedLNLinearV1(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with _nvtx_range("FusedLNLinear_V1"):
            ...
            raw_output = F.linear(x_2d, self.W_new)
            denominator_hip.denominator_normalize(x_2d, raw_output, self.b_new, self.h, self.eps)
...
class FusedLNLinearV3(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with _nvtx_range("FusedLNLinear_V3"):
            ...
            raw_output = F.linear(x_2d, self.W_new)
            denominator_hip.denominator_normalize_welford_512(x_2d, raw_output, self.b_new, self.h, self.eps)
...
class FusedRMSNormLinearV1(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with _nvtx_range("FusedRMSNormLinear_V1"):
            ...
            raw_output = F.linear(x_2d, self.W_new)
            denominator_hip.rmsnorm_normalize(x_2d, raw_output, self.b_new, self.h, self.eps)
...
class FusedRMSNormLinearV3(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with _nvtx_range("FusedRMSNormLinear_V3"):
            ...
            raw_output = F.linear(x_2d, self.W_new)
            denominator_hip.rmsnorm_normalize_512(x_2d, raw_output, self.b_new, self.h, self.eps)
...
def fused_rmsnorm_linear_forward_v1(
    ...
    denominator_hip.rmsnorm_normalize(x_2d, raw_output, b_new, h, eps)
...
def fused_rmsnorm_linear_forward_v3(
    ...
    raw_output = F.linear(x_2d, W_new)
    denominator_hip.rmsnorm_normalize_512(x_2d, raw_output, b_new, h, eps)
...
def fused_ln_linear_forward_v1(
    ...
    raw_output = F.linear(x_2d, W_new)
    denominator_hip.denominator_normalize(x_2d, raw_output, b_new, h, eps)
...
def fused_ln_linear_forward_v3(
    ...
    raw_output = F.linear(x_2d, W_new)
    denominator_hip.denominator_normalize_welford_512(x_2d, raw_output, b_new, h, eps)
